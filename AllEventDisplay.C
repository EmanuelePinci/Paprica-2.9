...
void AllEventDisplay(TString myfile, double soglia_ADC) {
   ...
   // --- Trascription of the map (X = ROW, Y = COL) ---
    ...
    TH1D *htc = new TH1D("hTotalCharge", "Distribuzione Carica Totale;Carica Totale [ADC];Conteggi", 140, 0, 0);
    ...
    for(int i = 0; i < tree->GetEntries(); i++){
        ...
        if(nhit>6){
            ...
            // Filling histo of total charge and reset ChargeTot
    ...
    //TCanvas *ch = new TCanvas("ch", "Distribution of total charge", 800, 600);
    htc->Draw();
    htc->Write();
    //ch->Draw();
    //ch->Write();
