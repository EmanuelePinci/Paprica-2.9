...
// We define conversion factor to ADC counts to photoelectron

const double GAIN_ADC_per_PHE = 300.0;
...
void AllEventDisplay(TString myfile, double soglia_ADC) {
    ...
    // We open a new .txt file for saving the total charge for next histo
   ...
   // --- Trascription of the map (X = ROW, Y = COL) ---
    ...
    // We define a variable to save the total charge
    double total_charge = 0.0; // <-- [PE]
    int id=0;
    double carica_hg=0.0;
    double carica_hg_phe=0.0;

    // We also collect the RMS of the histos to analyze the correlation

    double RMSx = 0.0, RMSy = 0.0;
    
    // We also define some ausiliary variables
    ...
    // 4. Data extraction and booking of the histos
    
    // Now we cycle on all tree's entries and book histo of charge on single SiPM
    
    gStyle->SetPaintTextFormat(".0f"); // Setting number format

    outFileTxt << "// Event_ID" << "\t\t" << "Total_Charge [PHE]" << "\t\t" << "RMSx" << "\t\t" << "RMSy" << endl; // Setting format intestation
    ...
    for(int i = 0; i < tree->GetEntries(); i++){
        ...
        if(nhit > 6 && nhit < 50){ // We discard too high value of nhit becaues of probable over tension
            ...
            for (size_t j = 0; j < iSiPM->size(); j++) {
                ...
                if (id >= 0 && id < 128 && fisX[id] != -1) {
                    ...
                    if (carica_hg > soglia_ADC) {
                        ...
                        carica_hg_phe= carica_hg/GAIN_ADC_per_PHE; // <-- Conversion

                        // We compute the total charge only for SiPM in interesting events
                        total_charge += carica_hg_phe;
            ...
            // We save the RMS values for the filled histo
            RMSx = hMap->GetRMS(1); // X-axis RMS 
            RMSy = hMap->GetRMS(2); // Y-axis RMS 

            // We save the value in .txt file and reset it 
            outFileTxt << eventNumber << "\t\t" << total_charge << "\t\t" << RMSx << "\t\t" << RMSy << endl;
            total_charge = 0.0;
            
            // 6. Configuration and drawing
            //hMap->SetStats(0);           
    ...
    // 8. When the cicle ends, we close the outfiles
    outFile->Close();
    outFileTxt.close();
    ...
    cout << "Gain used for ADC -> PHE conversion: " << GAIN_ADC_per_PHE << "ADC/PHE" << endl;
