...
void ExtractDAC(string file_name, double target_gain){
    ...
    /* We initialize two different vectors that identify the two CITIROC and we read the channel as 
       ...
       vector<int> CITIROC2(32, 0);
       vector<int> CITIROC3(32,0);
    ...
    while(getline(infile, line)){
        ...
        if(ss >> i_sipm >> p0 >> p1){ // <-- We read the values ordinately
            ...
            // For CITIROC2 we have: 64  --> 0
            //                       95  --> 31
            // For CITIROC3 we have: 96  --> 0
            //                       127 --> 31

            if(i_sipm >= 64 && i_sipm <= 95){
                 int iglbsipm = i_sipm - 64;
                CITIROC2[iglbsipm] = static_cast<int>(round(VDAC));
            }else if(i_sipm >= 96 && i_sipm <= 127){
                int iglbsipm = i_sipm - 96;
                CITIROC3[iglbsipm] = static_cast<int>(round(VDAC));
            }
    ...
    if(outfile.is_open()){
        ...
        outfile << "//i_SiPM" << "\t" << "iglbSiPM" << "\t" << "CITIROC" << "\t" << "DAC" << endl;
        for(int i_SiPM = 0; i_SiPM < 128; i_SiPM++){
            if(i_SiPM < 64){
                if(i_SiPM <= 31){
                    outfile << i_SiPM << "\t" << i_SiPM << "\t" << 0 << "\t" << 128 << endl;
                }else if(i_SiPM >= 32 && i_SiPM <=63){
                    outfile << i_SiPM << "\t" << i_SiPM - 32 << "\t" << 1 << "\t" << 128 << endl;
                }
            }else{
                if(i_SiPM >= 64 && i_SiPM <= 95){
                    outfile << i_SiPM << "\t" << i_SiPM - 64 << "\t" << 2 << "\t" << CITIROC2[i_SiPM - 64] << endl;
                }else{
                    outfile << i_SiPM << "\t" << i_SiPM - 96 << "\t" << 3 << "\t" << CITIROC3[i_SiPM - 96] << endl;
                }
            }
