...
void FitPeaks(std::string numrun, int VDAC) { 
    ...
    for (int i_sipm = 64; i_sipm <= 127; ++i_sipm){
        ...
        if(VDAC < 100){
            ...
            nPeaks = spec->Search(h, 10, "", 0.007);
        }else{
            h->Rebin(4);
            h->GetXaxis()->SetRangeUser(-70, 1200); // Limit the range to focus on the relevant part of the spectrum
            spec = new TSpectrum(50);
            nPeaks = spec->Search(h, 15, "", 0.01);
            h->GetXaxis()->UnZoom(); // Reset the zoom after peak search to fit the entire spectrum
        }
        ...
        for (int p = 0; p < peaksToFit; p++)
        {
            ...
            fitFunc->SetParameters(h->GetBinContent(h->FindBin(xc)), xc, range / 3.0);
