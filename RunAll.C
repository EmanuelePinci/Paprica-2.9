...
void RunAll(const vector<int>& mydac, const vector<string>& numrun, const vector<int>& cornerChannels, bool all_ch = true) {
    ...
    for(size_t i = 0; i < mydac.size(); ++i) {
        ...
        FitPeaks(numrun[i], mydac[i], all_ch, cornerChannels);
